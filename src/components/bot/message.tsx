...
const OptionalLinkedText = ({ children, url }) => {
  if (url) {
    return (
      <a href={url} target="_blank" className="underline">{children}</a>
    )
  }
    ...
    <div className={`rounded-xl ${them ? "bg-gray-300" : (typing ? "bg-red-300" : "bg-red-500")}`}>
      {youtubeVideoId ? (
        <a className="relative block" target="_blank" href={`https://www.youtube.com/watch?v=${youtubeVideoId}${startTime ? "&t=" + startTime : ""}`}>
          <div className="opacity-75 hover:opacity-100 absolute top-0 bottom-0 left-0 right-0 z-10 flex justify-center items-center">
            <PlayIcon className="w-[40%] text-white drop-shadow-md" />
          </div>
          <img
      ...
      {image ? <OptionalLinkedText url={url}><img className="w-full rounded-t-xl" src={image} /></OptionalLinkedText> : null}
      {(text || typing) ? (
        <p className={`${them ? "text-gray-500" : "text-white"} py-2 px-4 font-medium text-md`}>
          {typing ? (<>&nbsp;&nbsp;<span className="dot-flashing"></span>&nbsp;&nbsp;</>) : <OptionalLinkedText url={url}>{text}</OptionalLinkedText>}
        </p>
      ) : null}
    </div>
  </div>
